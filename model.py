...
class Encoder(nn.Module):
    def __init__(self, out_dim=1024):
        ...
        self.feature_ori = MLP_Res(in_dim=3, hidden_dim=32, out_dim=64)
        self.sa_module_1 = PointNet_SA_Module_KNN(512, 16, 64, [64, 128], group_all=False, if_bn=True, if_idx=True)
        self.transformer_1 = Transformer(128, dim=64)
        self.sa_module_2 = PointNet_SA_Module_KNN(128, 16, 128, [128, 256], group_all=False, if_bn=True, if_idx=True)
        self.transformer_2 = Transformer(256, dim=64)
        self.sa_module_3 = PointNet_SA_Module_KNN(None, None, 256, [512, out_dim], group_all=True, if_bn=True)
    ...
    def forward(self, point_cloud):
        ...
        """
        l0_xyz = point_cloud
        l0_points = self.feature_ori(point_cloud.contiguous())

        l1_xyz, l1_points = self.sa_module_1(l0_xyz, l0_points)
...
class Decoder(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, up_factors=None):
        ...
        self.num_p0 = num_p0
        self.decoder_coarse = GHG(dim_feat=dim_feat, num_pc=num_pc)
        ...
        for i, factor in enumerate(up_factors):
            uppers.append(GHGS(dim_feat=dim_feat, up_factor=factor, i=i, radius=radius))
...
class ANFF(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, up_factors=[2,2]):
        """
        Args:
        ...
        """
        super(ANFF, self).__init__()
        self.encoder = Encoder(out_dim=dim_feat)
...
if __name__ == "__main__":
    ...
    INPUT = torch.rand(16,2048,3).to(device)
    xx = ANFF().to(device)
    yy = xx(INPUT)
    print(yy)