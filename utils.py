...
class global_knn(nn.Module):
    def __init__(self, input_dim=64):
        super(global_knn, self).__init__()
        self.mlp1_zsy = MLP_Res(in_dim=input_dim, out_dim=input_dim)
        self.mlp2_zsy = MLP_Res(in_dim=input_dim, out_dim=input_dim)

    def forward(self, nsample, points, new_points, include_self=True):
        pad = 0 if include_self else 1
        points_mlp = self.mlp1_zsy(points.permute(0, 2, 1))
        new_points_mlp = self.mlp2_zsy(new_points.permute(0, 2, 1))
        sqrdists = torch.bmm(new_points_mlp.permute(0, 2, 1), points_mlp)  # B, S, N
        idx = torch.argsort(sqrdists, dim=-1, descending=False)[:, :, pad: nsample + pad]
        return idx.int()
...
def sample_and_group_knn(xyz, points, npoint, k, use_xyz=True, idx=None):
    ...
    """
    xyz_flipped = xyz.permute(0, 2, 1).contiguous() # (B, N, 3)
    points_flipped = points.permute(0, 2, 1).contiguous() # (B, N, f)
    new_xyz = gather_operation(xyz, furthest_point_sample(xyz_flipped, npoint)) # (B, 3, npoint)
    new_points = gather_operation(points, furthest_point_sample(xyz_flipped, npoint))  # (B, 3, npoint)
    new_points_backup = new_points.permute(0, 2, 1).contiguous()
    # if idx is None:
    idx1 = query_knn(k, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous())

    # idx2 = query_knn(k, points_flipped, new_points.permute(0, 2, 1).contiguous())

    grouped_xyz = grouping_operation(xyz, idx1) # (B, 3, npoint, nsample)
    grouped_xyz -= new_xyz.unsqueeze(3).repeat(1, 1, 1, k)

    if points is not None:
        grouped_points = grouping_operation(points, idx1) # (B, f, npoint, nsample)
        grouped_points -= new_points.unsqueeze(3).repeat(1, 1, 1, k)
        # global_points = grouping_operation(points, idx2)
        # global_points -= new_points.unsqueeze(3).repeat(1, 1, 1, k)
        if use_xyz:
            # new_points = torch.cat([grouped_xyz, grouped_points, global_points], 1)
            new_points = torch.cat([grouped_xyz, grouped_points], 1)
        else:
            new_points = grouped_points
    else:
    ...
    return new_xyz, new_points, points_flipped, new_points_backup
...
class PointNet_SA_Module_KNN(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True, group_all=False, use_xyz=True, if_idx=False):
        ...
        self.global_knn = global_knn(in_channel)
        self.npoint = npoint
        self.nsample = nsample
        self.mlp = mlp
        self.group_all = group_all
        ...
        self.if_idx = if_idx
        if use_xyz:
            in_channel1 = 2 * in_channel + 3
            in_channel2 = in_channel + 3
        last_channel1 = in_channel1
        self.mlp_conv1 = []
        for out_channel in mlp[:-1]:
            self.mlp_conv1.append(Conv2d(last_channel1, out_channel, if_bn=if_bn))
            last_channel1 = out_channel
        self.mlp_conv1.append(Conv2d(last_channel1, mlp[-1], if_bn=False, activation_fn=None))
        self.mlp_conv1 = nn.Sequential(*self.mlp_conv1)

        last_channel2 = in_channel2
        self.mlp_conv2 = []
        for out_channel in mlp[:-1]:
            self.mlp_conv2.append(Conv2d(last_channel2, out_channel, if_bn=if_bn))
            last_channel2 = out_channel
        self.mlp_conv2.append(Conv2d(last_channel2, mlp[-1], if_bn=False, activation_fn=None))
        self.mlp_conv2 = nn.Sequential(*self.mlp_conv2)
    ...
    def forward(self, xyz, points, idx=None):
        ...
        """
        if self.group_all:
            new_xyz, new_points = sample_and_group_all(xyz, points, self.use_xyz)
            new_points = self.mlp_conv2(new_points)
        else:
            new_xyz, new_points, points_flipped, new_points_backup = sample_and_group_knn(xyz, points, self.npoint, self.nsample, self.use_xyz, idx=idx)
            idx2 = self.global_knn(self.nsample, points_flipped, new_points_backup.contiguous())
            global_points = grouping_operation(points, idx2)
            global_points -= new_points_backup.permute(0, 2, 1).unsqueeze(3).repeat(1, 1, 1, self.nsample)
            new_points = torch.cat([new_points, global_points], 1)
